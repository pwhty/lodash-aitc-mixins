 ...
 * collection. The callback is bound to `thisArg` and invoked with four arguments; (value, index|key, level, collection).
 *
 ...
 * // => ['barney', ['fred']]
 */
_.mixin({'deepMap' : function(collection, callback, thisArg, level){
    level = level || 0;
    ...
    return _.map(collection, function (value, index, collection) {
        ...
        if(_.isArray(value))
            return _.deepMap(value, callback, thisArg, level + 1);
 ...
 * recursevely. The callback is bound to `thisArg` and invoked with four arguments; (value, key, level, object).
 *
 * WARNING: The following two features (_.pluck and _.where styles) are not working as described below. They should
 *          kick in only on element children and continue the recursion on array or object children. As such, the second
 *          example does not work as stated below.
 *
 ...
 * // => { 'fred': 40, 'pebbles': 1 }
 */
_.mixin({'deepMapValues' : function(object, callback, thisArg, level){
    level = level || 0;
    callback = _.createCallback(callback, thisArg, 3);
    return _.mapValues(object, function (value, key, collection) {
        // if it's an array or object, map recursively inside it
        if(_.isArray(value) || _.isObject(value)){
            return _.deepMapValues(value, callback, thisArg, level + 1);
        }
        // if it's an element, run it through the callback
        else{
            return callback(value, key, level, collection);
        }
    });
}});

/**
 * Reduces a collection to a value which is the accumulated result of running each element in the collection through
 * the callback, where each successive callback execution consumes the return value of the previous execution. The
 * callback function which has access to the nesting level of the element. If `accumulator` is not provided, the first
 * element of the collection will be used as the initial `accumulator` value. The callback is bound to `thisArg`
 * and invoked with five arguments; (accumulator, value, index|key, level, collection).
 *
 * @static
 * @alias foldl, inject
 * @category Collections
 ...
 */
_.mixin({'deepReduce':function reduce(collection, callback, accumulator, thisArg, level) {
    level = level || 0;
    ...
    return _.reduce(collection, function (accumulator, value, key, collection) {
        ...
        if(_.isArray(value) || _.isObject(value)){
            return _.deepReduce(value, callback, accumulator, thisArg, level + 1);
        }
        // if it's an element, run it through the callback
        else{
            return callback(accumulator, value, key, level, collection);
        }
    }, accumulator);
}});
 ...
 * into array children. Concatenation can also be redefined through a callback function which has access to the nesting
 * level. The callback is bound to `thisArg` and invoked with three arguments; (collectionValue, sourceValue, level).
 *
 * @static
 * @category Collections
 * @param {Array} collection The destination array.
...
_.mixin({'deepMergeArray' : function(collection, source, callback, thisArg, level){
    ...
    return _.map(collection, function(item, index, collection){
        // if it's an array, map recursively inside it
        if(_.isArray(item))
            return _.deepMergeArray(collection[index], (source && source[index]) || undefined, callback, thisArg, level + 1);
        // if it's an element, run the callback
        else
            return callback(collection[index], (source && source[index]) || undefined, level);
    });
}});
